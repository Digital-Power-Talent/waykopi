<div class="py-8 max-w-7xl mx-auto space-y-8">
                            ...
                            <tr class="hover:bg-[var(--color-bg-base)]/50 transition-colors">
                                ...
                                <td class="py-3 px-4">
                                    ...
                                    @if($order->shipment?->tracking_number)
                                        <span class="text-[10px] text-emerald-400 block font-mono">Resi: {{ $order->shipment->tracking_number }}</span>
                                    @endif
                                </td>
                                <td class="py-3 px-4">
                                    @php
    ...
    @if($showOrderModal && $selectedOrder)
        <div class="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div class="bg-[var(--color-bg-surface)] p-6 rounded-[var(--radius-md)] border border-[var(--color-coffee-brown)] shadow-2xl max-w-lg w-full space-y-5 font-mono text-xs">
                <div class="flex items-center justify-between border-b border-[var(--color-coffee-brown)] pb-3">
                    <h3 class="font-display font-bold text-base text-[var(--color-text-primary)] font-sans">
                        Update Status Pesanan #{{ $selectedOrder->order_number }}
                    </h3>
                    <button wire:click="closeOrderModal" class="text-[var(--color-text-muted)] hover:text-white">&times;</button>
                </div>
                ...
                <div class="p-3 bg-[var(--color-bg-base)] border border-[var(--color-coffee-brown)]/40 rounded space-y-2">
                    ...
                    <p><strong>Kurir:</strong> {{ $selectedOrder->courier_name }}</p>
